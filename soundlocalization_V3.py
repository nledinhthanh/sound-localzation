...
mic_pos = np.array([[0, 0, 0], [0.13, 0, 0], [0.065, 0.113, 0]]) * 100
...
with sd.InputStream(callback=clap_detect_callback, blocksize=1024, samplerate=fs, channels=1):
    print("Recording started. Clap to detect direction and distance...")
    ...
    print("Direction of arrival (direction): ", direction, "degrees")
    print("Distance of the sound source: ", distance, "meters")
    
    # Plot the recorded audio data from each microphone
    plt.figure()
    plt.title("Recorded audio data from each microphone")
    for i in range(3):
        plt.subplot(3, 1, i + 1)
        plt.plot(data[i])
